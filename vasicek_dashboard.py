# Vasicek Interest Rate Model — SBP Policy Rate Simulation Dashboard
...
# Persist dark/light preference across reruns
if "dark_mode" not in st.session_state:
    st.session_state.dark_mode = True
D = st.session_state.dark_mode

# All UI colours in one dict — swaps between dark and light values
T = {
...
# Global styles + animated particle network canvas injected into the page
st.markdown(f"""
<style>
...
.insight-text{{font-family:'Outfit',sans-serif;font-size:12.5px;color:{T['chart_title']};line-height:1.7;}}
...
@st.cache_data
def load_data(path):
    # Read CSV, strip whitespace from headers, sort by date, convert rate to decimal
    df = pd.read_csv(path)
...
def fetch_live_sbp_rate():
    # Scrapes sbp.org.pk for the current policy rate; caches result for 24 hours
    try:
        ...
        if match:
            return float(match.group(1)) / 100, datetime.now().strftime("%d %b %Y"), None
        return None, None, "Rate not found on SBP page"
    except Exception as e:
        return None, None, str(e)

def append_live_rate(df, live_rate):
    # Appends today's live rate if it differs from the last CSV entry; otherwise updates the date
    today = pd.Timestamp(datetime.now().date())
    if abs(live_rate - df['Rate'].iloc[-1]) > 0.0001:
        df = pd.concat([df, pd.DataFrame({'Date': [today], 'Rate': [live_rate]})], ignore_index=True)
    else:
        df.loc[df.index[-1], 'Date'] = today
    return df

def run_vasicek(rates, a, years, simulations):
    # Euler-Maruyama discretisation of dr = a(b - r)dt + σ√dt·Z
    # b and σ are estimated from historical data; seed is fixed for reproducibility
    np.random.seed(42)
    dt = 1/12
    steps = int(years / dt)
    b, sigma, r0 = rates.mean(), rates.std(), rates.iloc[-1]
    sim = np.zeros((simulations, steps))
    sim[:, 0] = r0
...
# ── Live Rate Badge ─────────────────────────────────────────────────────────────
# Shows green badge with live rate if scrape succeeded, red warning if it failed
live_rate, live_date, live_error = fetch_live_sbp_rate()
if live_error:
    st.markdown(f"""<div style="position:fixed;bottom:20px;left:20px;z-index:9999;
        background:{'#1a0a0a' if D else '#fff3f3'};border:1px solid #ef4444;border-radius:10px;
    ...
    st.markdown(f"""<div style="position:fixed;bottom:20px;left:20px;z-index:9999;
        background:{'#0a1a0a' if D else '#f0fff4'};border:1px solid #10b981;border-radius:10px;
        padding:10px 16px;font-family:'JetBrains Mono',monospace;font-size:10px;
...
# ── Controls ────────────────────────────────────────────────────────────────────
# Sliders stage values; simulation only reruns when RUN is clicked
if "run_params" not in st.session_state:
    st.session_state.run_params = dict(years=10, simulations=500, sample_paths=10, a_val=0.5)
...
with left:
    # Simulation chart: sample paths + 68/95% CI bands + glowing mean line
    ...
    for i in range(sample_paths):
        fig_sim.add_trace(go.Scatter(x=time, y=sim[i]*100, line=dict(color=T['path_color'], width=0.6), showlegend=False, hoverinfo='skip'))
    ...
    for w, op in [(8,0.04),(4,0.08),(2,0.2)]:
        fig_sim.add_trace(go.Scatter(x=time, y=mean_path*100, line=dict(color=T['mean_color'], width=w), opacity=op, showlegend=False, hoverinfo='skip'))
    ...
    fig_sim.update_layout(height=400, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(color=T['tick_color'],size=11),
        legend=dict(bgcolor='rgba(0,0,0,0)', bordercolor=T['legend_border'], borderwidth=1, font=dict(size=10), orientation='h', y=-0.18),
        margin=dict(l=40,r=20,t=10,b=55),
        xaxis=dict(gridcolor=T['grid_color'], zerolinecolor=T['grid_color'], title='Years', title_font=dict(size=11)),
    ...
    The current rate of <strong>{r0*100:.1f}%</strong> sits <strong>{'above' if above_mean else 'below'} the long-run mean of {b*100:.1f}%</strong>,
    creating <strong>{reversion_bias} mean-reversion pressure</strong>. At {years} years, the 95% CI spans <strong>{ci_width:.1f}pp</strong>.
    </div></div>""", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)

with right:
    # Historical rate area chart
    ...
    fig_hist.update_layout(height=185, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(color=T['tick_color'],size=10), showlegend=False,
        margin=dict(l=40,r=10,t=5,b=30), xaxis=dict(gridcolor=T['grid_color'], zerolinecolor=T['grid_color']),
        yaxis=dict(gridcolor=T['grid_color'], zerolinecolor=T['grid_color'], ticksuffix='%'))
    st.plotly_chart(fig_hist, use_container_width=True)
    st.markdown("</div>", unsafe_allow_html=True)

    # Terminal distribution: histogram of all simulated end-rates with mean and current rate markers
    st.markdown(f'<div class="chart-wrap"><div class="chart-header"><span class="chart-title">Terminal Distribution — {years}Y</span><span class="chart-tag">SIMULATION OUTPUT</span></div>', unsafe_allow_html=True)
    fig_dist = go.Figure()
    ...
    fig_dist.update_layout(height=185, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(color=T['tick_color'],size=10), showlegend=False,
        margin=dict(l=40,r=10,t=5,b=30), bargap=0.02,
        xaxis=dict(gridcolor=T['grid_color'], zerolinecolor=T['grid_color'], ticksuffix='%'),
        yaxis=dict(gridcolor=T['grid_color'], zerolinecolor=T['grid_color']))
    st.plotly_chart(fig_dist, use_container_width=True)
    prob_above = np.mean(sim[:,-1]*100 > r0*100) * 100
    st.markdown(f"""<div class="insight-box"><div class="insight-text">
    After <strong>{years} years</strong>, <strong>{prob_above:.0f}% of paths</strong> end above the current rate.
    Median: <strong>{np.median(final_rates):.1f}%</strong> · 90% range: <strong>{np.percentile(final_rates,5):.1f}% – {np.percentile(final_rates,95):.1f}%</strong>.
    </div></div>""", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)

# Fan chart: historical + forecast cone joined into one view
...
fig_fan.add_trace(go.Scatter(x=hist_years, y=df['Rate']*100, line=dict(color=T['hist_line'], width=2), name='Historical Rate', fill='tozeroy', fillcolor=T['hist_fill']))
for lo, hi, alpha, label in [(5,95,0.06,'90% Range'),(10,90,0.10,'80% Range'),(20,80,0.14,'60% Range'),(35,65,0.20,'30% Range')]:
    r,g,b_c = (124,58,237) if D else (37,99,235)
    fig_fan.add_trace(go.Scatter(x=np.concatenate([time,time[::-1]]), y=np.concatenate([np.percentile(sim,hi,axis=0)*100, np.percentile(sim,lo,axis=0)[::-1]*100]),
        fill='toself', fillcolor=f'rgba({r},{g},{b_c},{alpha})', line=dict(color='rgba(0,0,0,0)'), name=label, hoverinfo='skip'))
fig_fan.add_trace(go.Scatter(x=time, y=mean_path*100, line=dict(color=T['mean_color'], width=2.5), name='Mean Forecast'))
fig_fan.add_vline(x=0, line=dict(color=T['current_color'], width=1.5, dash='dash'), annotation_text="  Today", annotation_font_color=T['current_color'], annotation_font_size=11)
fig_fan.add_hline(y=b*100, line=dict(color=T['vline_color'], width=1, dash='dot'), annotation_text=f"  Long-run mean {b*100:.1f}%", annotation_font_color=T['vline_color'], annotation_font_size=10)
fig_fan.update_layout(height=340, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(color=T['tick_color'],size=11),
    legend=dict(bgcolor='rgba(0,0,0,0)', bordercolor=T['legend_border'], borderwidth=1, font=dict(size=10), orientation='h', y=-0.22),
    margin=dict(l=40,r=20,t=10,b=65),
    xaxis=dict(gridcolor=T['grid_color'], zerolinecolor=T['grid_color'], title='Years (negative = past, positive = forecast)', title_font=dict(size=10)),
    yaxis=dict(gridcolor=T['grid_color'], zerolinecolor=T['grid_color'], ticksuffix='%'))
...
st.markdown(f"""<div class="insight-box"><div class="insight-text">
The 90% band is <strong>{fan_spread_1y:.1f}pp wide at 1 year</strong>, expanding to <strong>{fan_spread_end:.1f}pp at {years} years</strong> —
uncertainty compounds with time. All paths are pulled toward the long-run mean of <strong>{b*100:.1f}%</strong> regardless of where they start.
</div></div>""", unsafe_allow_html=True)
st.markdown("</div>", unsafe_allow_html=True)

# Sensitivity table: reruns the model at 6 values of 'a' to show how it affects the forecast
st.markdown(f'<div class="chart-wrap"><div class="chart-header"><span class="chart-title">Parameter Sensitivity — Mean Reversion Speed (a)</span><span class="chart-tag">SCENARIO ANALYSIS</span></div>', unsafe_allow_html=True)
rows_html = ""
for a_test in [0.1, 0.3, 0.5, 1.0, 1.5, 2.0]:
    sim_test, _, _, _, _ = run_vasicek(df['Rate'], a_test, max([1,3,5,10]), simulations)
    cells = ""
    for h in [1, 3, 5, 10]:
        idx = int(h*12) - 1
        ...
        cls = ' class="highlight"' if abs(a_test - a_val) < 0.01 else ''
...
<table class="sens-table">
  <thead><tr><th>Speed (a)</th><th>1Y Mean (5–95%)</th><th>3Y Mean (5–95%)</th><th>5Y Mean (5–95%)</th><th>10Y Mean (5–95%)</th></tr></thead>
  <tbody>{rows_html}</tbody>
</table>
<div style="font-family:'Outfit',sans-serif;font-size:11px;color:{T['label_color']};margin-top:10px;line-height:1.6;">
Higher <em>a</em> = faster reversion → tighter bands. Lower <em>a</em> = slower reversion → wider uncertainty. Highlighted row = current setting.
</div>""", unsafe_allow_html=True)
st.markdown("</div>", unsafe_allow_html=True)

# Export: forecast time series and model parameters as downloadable CSVs
...
export_df = pd.DataFrame({
    'Year': np.round(time,4), 'Mean_Rate_%': np.round(mean_path*100,4),
    'CI_68_Low_%': np.round(lower_68*100,4), 'CI_68_High_%': np.round(upper_68*100,4),
    'CI_95_Low_%': np.round(lower_95*100,4), 'CI_95_High_%': np.round(upper_95*100,4),
})
...
with exp_col1:
    st.download_button("⬇ Download Forecast Data (CSV)", export_df.to_csv(index=False).encode(),
                       f"vasicek_forecast_{years}y_{simulations}sims.csv", 'text/csv', use_container_width=True)
with exp_col2:
    st.download_button("⬇ Download Model Parameters (CSV)", meta_df.to_csv(index=False).encode(),
                       "vasicek_parameters.csv", 'text/csv', use_container_width=True)
st.markdown("</div>", unsafe_allow_html=True)
